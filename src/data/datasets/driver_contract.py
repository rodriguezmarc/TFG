...
from data.prompts.driver_contract import PromptMetadataAdapter
...
class DatasetDriver(PromptMetadataAdapter, Protocol):
    ...
    ########################################
    Definition:
    Describe the structural contract implemented by dataset drivers.
    ---
    Results:
...
class BaseCardiacDatasetDriver(ABC):
    ...
    def emit_progress(self, message: str) -> None:
        ...
        ########################################
        Definition:
        Emit a user-facing progress line for critical pipeline milestones.
        ---
        Params:
        message: Progress text to display.
        ---
        Results:
        Prints one normalized progress line.
        ########################################
        """
        print(message)
    ...
    def build_rows(
        ...
        self.emit_progress(f"Found {len(case_paths)} {self.dataset_name.upper()} cases.")
        preprocessed_count = 0
        prompt_count = 0
        for case_path in case_paths:
            es_slice, mask_slice, ef, metadata = self.preprocess_case(case_path)
            preprocessed_count += 1
            img_filename = self.save_case_outputs(es_slice, mask_slice, metadata, images_root)
            prompt_text = (
                self.render_prompt(metadata, ef, modality)
                if prompt_fn is None
                else prompt_fn(metadata, ef, modality)
            )
            prompt_count += 1
            rows.append(self.build_row(img_filename, metadata, prompt_text, modality))
        self.emit_progress(f"Preprocessing completed for {preprocessed_count} cases.")
        self.emit_progress(f"Prompt generation completed for {prompt_count} cases.")
        self.emit_progress(f"All rows were built successfully.")
        return rows
